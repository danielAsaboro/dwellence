...
async function withApi(run) {
  const store = createStore(
    join(
      mkdtempSync(join(tmpdir(), "location-evidence-evidence-")),
      "state.sqlite",
    ),
  );
...
async function registerSensor(url, contributor, metadata) {
  const challenge = await json(url, "/api/sensors/registration-challenge", {
    method: "POST",
    headers: { authorization: `Bearer ${contributor.token}` },
    body: JSON.stringify(metadata),
  });
  return json(url, "/api/sensors/register", {
    method: "POST",
    headers: { authorization: `Bearer ${contributor.token}` },
    body: JSON.stringify({
      ...metadata,
      registrationChallengeId: challenge.body.challengeId,
      walletPublicKey: contributor.keyPair.publicKey.toHex(),
      walletSignature: contributor.keyPair
        .sign(new TextEncoder().encode(challenge.body.message))
        .toHex(),
    }),
  });
...
describe("signed sensor readings", () => {
  it("accepts one valid physical-sensor payload and rejects its replay", async () => {
    await withApi(async (url) => {
      ...
      const rawPublicKey = publicKey
        ...
        .subarray(-32)
        .toString("hex");
      const sensor = await registerSensor(url, contributor, {
      ...
      const reading = {
        ...payload,
        signature: signedReading(privateKey, payload),
      };
      const accepted = await json(
        url,
        `/api/sensors/${sensor.body.id}/readings`,
  ...
  it("rejects sensor registration without a wallet signature over its metadata", async () => {
    await withApi(async (url) => {
      ...
      const result = await json(url, "/api/sensors/register", {
        method: "POST",
        headers: { authorization: `Bearer ${contributor.token}` },
        body: JSON.stringify({
          publicKey: publicKey
