...
async function withApi(run) {
  const store = createStore(
    join(mkdtempSync(join(tmpdir(), "location-evidence-api-")), "state.sqlite"),
  );
...
async function authenticate(url, role) {
  ...
  const challenge = await json(url, "/api/auth/challenge", {
    method: "POST",
...
describe("private request API", () => {
  it("serves health and the built Mini App from one production process", async () => {
    ...
    writeFileSync(
      ...
      "<!doctype html><title>Dwellence</title>",
    );
    ...
    try {
      ...
      expect(await (await fetch(`${base}/`)).text()).toContain(
        "<title>Dwellence</title>",
      );
      expect(
        (await fetch(`${base}/assets/app.js`)).headers.get("content-type"),
  ...
  it("allows configured browser origins to preflight authenticated API requests", async () => {
    const store = createStore(
      join(
        mkdtempSync(join(tmpdir(), "location-evidence-cors-")),
        "state.sqlite",
      ),
    );
    const server = createApp({
  ...
  it("serves a controlled connectivity probe without fabricated results", async () => {
    await withApi(async (url) => {
      ...
      const upload = await fetch(`${url}/probe/upload`, {
        method: "POST",
  ...
  it("accepts only allowlisted minimal analytics without sensitive fields", async () => {
    await withApi(async (url) => {
      expect(
        (
          await json(url, "/api/analytics", {
            method: "POST",
            body: JSON.stringify({
              event: "app_opened_outside_nimiq_pay",
              clientId: "client_1234567890123456",
            }),
          })
        ).status,
      ).toBe(201);
      const rejected = await json(url, "/api/analytics", {
        method: "POST",
        body: JSON.stringify({
          event: "request_created",
          clientId: "client_1234567890123456",
          walletAddress: "NQ secret",
        }),
      });
  ...
  it("records consented anonymous tester feedback and rejects unconsented submissions", async () => {
    await withApi(async (url) => {
      const accepted = await json(url, "/api/feedback", {
        method: "POST",
        body: JSON.stringify({
          clientId: "client_1234567890123456",
          rating: 4,
          comment: "The payment explanation was clear.",
          evidenceConsent: true,
        }),
      });
      expect(accepted.status).toBe(201);
      expect(accepted.body.receiptId).toMatch(/^feedback_/);
      expect(
        (
          await json(url, "/api/feedback", {
            method: "POST",
            body: JSON.stringify({
              clientId: "client_1234567890123456",
              rating: 4,
  ...
  it("rate limits repeated wallet challenge requests without claiming person uniqueness", async () => {
    await withApi(async (url) => {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        expect(
          (
            await json(url, "/api/auth/challenge", {
              method: "POST",
              body: JSON.stringify({ role: "seeker", address }),
            })
          ).status,
        ).toBe(201);
      }
      const limited = await json(url, "/api/auth/challenge", {
        method: "POST",
        body: JSON.stringify({ role: "seeker", address }),
      });
  ...
  it("rejects request creation without an authenticated seeker session", async () => {
    await withApi(async (url) => {
      const result = await json(url, "/api/requests", {
        method: "POST",
        body: "{}",
      });
  ...
  it("rejects an authenticated request that lacks a signature over its exact payload", async () => {
    await withApi(async (url) => {
      ...
      const unsigned = await json(url, "/api/requests", {
        method: "POST",
        headers: { authorization: `Bearer ${seeker.token}` },
        body: JSON.stringify(draft),
      });
  ...
  it("rejects a signed request with a malformed invited Nimiq address", async () => {
    await withApi(async (url) => {
      ...
      const result = await createSignedRequest(url, seeker, {
        location: { latitude: 6.5, longitude: 3.3 },
        invitedContributor: "NQTHISISNOTAVALIDADDRESS",
        windowStartsAt: Date.now(),
        windowEndsAt: Date.now() + 60_000,
        priceLuna: 1000,
      });
  ...
  it("rejects signed requests whose precise coordinates are outside Earth bounds", async () => {
    await withApi(async (url) => {
      ...
      const contributor = await authenticate(url, "contributor");
      const result = await createSignedRequest(url, seeker, {
        location: { latitude: 91, longitude: 3.3 },
        invitedContributor: contributor.address,
        windowStartsAt: Date.now(),
        windowEndsAt: Date.now() + 60_000,
        priceLuna: 1000,
  ...
  it("rejects a request without supported required evidence categories", async () => {
    await withApi(async (url) => {
      ...
      const result = await json(url, "/api/requests/challenge", {
        method: "POST",
        headers: { authorization: `Bearer ${seeker.token}` },
        body: JSON.stringify(draft),
      });
  ...
  it("rejects malformed sensor public-key hex before issuing a binding challenge", async () => {
    await withApi(async (url) => {
      const contributor = await authenticate(url, "contributor");
      const result = await json(url, "/api/sensors/registration-challenge", {
        method: "POST",
  ...
  it("stores only a digest of a consented origin-scoped device handle", async () => {
    const store = createStore(
      join(
        mkdtempSync(join(tmpdir(), "location-evidence-device-")),
        "state.sqlite",
      ),
    );
    const server = createApp({
    ...
    try {
      ...
      const challenge = await json(
        ...
        {
          method: "POST",
          body: JSON.stringify({ role: "contributor", address }),
        },
      );
        ...
        "/api/auth/verify",
        {
          ...
          body: JSON.stringify({
            ...
            signature: keyPair
              .sign(new TextEncoder().encode(challenge.body.message))
              .toHex(),
            deviceHandle: rawHandle,
          }),
        },
      );
      expect(verified.status).toBe(200);
      const row = store
        .prepare("SELECT device_handle_digest FROM sessions")
        .get();
  ...
  it("creates an invitation-only request and permits exactly one contributor acceptance", async () => {
    await withApi(async (url) => {
      ...
      const internalIdAttempt = await json(
        url,
        `/api/invitations/${request.body.id}/accept`,
        {
          method: "POST",
          headers: { authorization: `Bearer ${contributor.token}` },
          body: "{}",
  ...
  it("expires an invitation after its measurement window ends", async () => {
    await withApi(async (url) => {
      ...
      const request = await createSignedRequest(url, seeker, {
        location: { latitude: 6.5, longitude: 3.3 },
        invitedContributor: contributor.address,
        windowStartsAt: Date.now() - 60_000,
        windowEndsAt: Date.now() - 1,
        priceLuna: 1000,
      });
      const accept = await json(
        url,
