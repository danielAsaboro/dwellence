  ...
  it('reports endpoint-relative download, upload, latency and jitter from measured durations', async () => {
    ...
    const result = await runConnectivityProbe({
      ...
      fetchImpl: async (url: string, init?: RequestInit) => {
        if (url.endsWith('/download?bytes=125000')) return new Response(new Uint8Array(125000))
        if (url.endsWith('/upload')) return new Response(null, { status: init?.method === 'POST' ? 204 : 405 })
