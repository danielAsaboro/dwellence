...
export async function runConnectivityProbe({
  ...
  try {
    ...
    const downloadDurationMs = now() - downloadStarted;
    ...
    const uploadMbps = rateMbps(payload.byteLength, uploadDurationMs);
    ...
    const latencyMs = Number(
      (pings.reduce((sum, value) => sum + value, 0) / pings.length).toFixed(2),
    );
    const jitterMs = Number(
      (
        pings.reduce((sum, value) => sum + Math.abs(value - latencyMs), 0) /
        pings.length
      ).toFixed(2),
    );
    const durationMs = Number(
      (
        downloadDurationMs +
        uploadDurationMs +
        pings.reduce((sum, value) => sum + value, 0)
      ).toFixed(2),
    );
