...
const databasePath = resolve(
  process.env.DATABASE_PATH ?? "./data/dwellence.sqlite",
);
const locationEncryptionKey = process.env.LOCATION_ENCRYPTION_KEY;
const probeEndpointUrl = process.env.PROBE_PUBLIC_URL;
...
if (!locationEncryptionKey || locationEncryptionKey.length < 24) {
  console.error(
    "Refusing to start: set LOCATION_ENCRYPTION_KEY to a unique 24+-character secret.",
  );
  process.exit(1);
}
