...
function requireSession(request, store, role) {
  ...
  const session = store.prepare('SELECT wallet_address, role FROM sessions WHERE token_digest = ? AND expires_at >= ?').get(sha256(token), Date.now())
...
function requestDigest({ location, invitedContributor, windowStartsAt, windowEndsAt, priceLuna }) {
...
function requestSignatureMessage(walletAddress, digest, challengeId, expiresAt) {
...
function calculateConnectivityScore(reading) {
  ...
  const jitter = Math.max(0, Math.min(100, (100 - reading.jitter_ms) / .98))
  return Math.round((download * 25 + upload * 15 + latency * 25 + jitter * 20) / 85)
}

function calculateComfortScore(reading) {
  const temperature = Math.max(0, 100 - Math.abs(reading.temperature_c - 24) / 12 * 100)
  const humidity = Math.max(0, 100 - Math.abs(reading.humidity_percent - 50) / 40 * 100)
...
export function createApp({ store, locationEncryptionKey, allowedOrigins = [], nimiqRpcUrl = process.env.NIMIQ_RPC_URL, transactionLookup = null, staticDirectory = null }) {
  if (!locationEncryptionKey || locationEncryptionKey.length < 24) throw new Error('LOCATION_ENCRYPTION_KEY is required')
  const staticRoot = staticDirectory ? resolve(staticDirectory) : null

  return createServer(async (request, response) => {
    try {
      const origin = request.headers.origin
      if (origin && allowedOrigins.length > 0 && !allowedOrigins.includes(origin)) return send(response, 403, { code: 'ORIGIN_FORBIDDEN' })
      ...
      if (request.method === 'POST' && url.pathname === '/api/auth/verify') {
        const { challengeId, publicKey, signature, address, deviceHandle } = await readJson(request)
        if (deviceHandle !== undefined && !/^[0-9a-f]{64}$/i.test(String(deviceHandle))) return send(response, 400, { code: 'INVALID_DEVICE_HANDLE' })
        const nonceRows = store.prepare("SELECT binding FROM nonces WHERE purpose = 'wallet-auth' AND consumed_at IS NULL").all()
        const record = nonceRows.map((row) => JSON.parse(row.binding)).find((row) => row.challengeId === challengeId)
        if (!record || canonicalAddress(address) !== record.walletAddress) return send(response, 401, { code: 'AUTH_CHALLENGE_INVALID' })
        const nonceRow = store.prepare("SELECT digest FROM nonces WHERE purpose = 'wallet-auth' AND binding = ?").get(JSON.stringify(record))
        const message = `Dwellence authentication\nRole: ${record.role}\nAddress: ${record.walletAddress}\nNonce: ${challengeId}\nExpires: ${new Date(record.expiresAt).toISOString()}`
        if (!nonceRow || !verifyNimiqWalletSignature(message, signature, publicKey, address)) return send(response, 401, { code: 'SIGNATURE_INVALID' })
        if (!consumeNonce(store, challengeId, 'wallet-auth', JSON.stringify(record))) return send(response, 401, { code: 'AUTH_CHALLENGE_USED_OR_EXPIRED' })
        const token = randomBytes(32).toString('base64url')
        store.prepare('INSERT INTO sessions (token_digest, wallet_address, role, device_handle_digest, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(sha256(token), record.walletAddress, record.role, deviceHandle ? sha256(deviceHandle) : null, Date.now() + 30 * 60_000, Date.now())
        return send(response, 200, { token, walletAddress: record.walletAddress, expiresAt: Date.now() + 30 * 60_000 })
      }

      if (request.method === 'POST' && url.pathname === '/api/requests/challenge') {
        ...
        return send(response, 201, { challengeId, message: requestSignatureMessage(session.wallet_address, digest, challengeId, expiresAt), expiresAt })
      }
      ...
      const acceptMatch = url.pathname.match(/^\/api\/invitations\/([^/]+)\/accept$/)
      ...
      if (request.method === 'POST' && cancelMatch) {
        const session = requireSession(request, store, 'seeker')
        if (!session) return send(response, 401, { code: 'SEEKER_AUTH_REQUIRED' })
        const updated = store.prepare("UPDATE requests SET status = 'cancelled' WHERE id = ? AND seeker_address = ? AND status = 'shared'").run(cancelMatch[1], session.wallet_address)
        if (updated.changes !== 1) return send(response, 409, { code: 'REQUEST_CANNOT_BE_CANCELLED' })
      ...
      const sensorChallengeMatch = url.pathname.match(/^\/api\/sensors\/([^/]+)\/challenge$/)
      if (request.method === 'POST' && sensorChallengeMatch) {
        const session = requireSession(request, store, 'contributor')
        if (!session) return send(response, 401, { code: 'CONTRIBUTOR_AUTH_REQUIRED' })
        const sensor = store.prepare('SELECT id FROM sensors WHERE id = ? AND operator_address = ?').get(sensorChallengeMatch[1], session.wallet_address)
        if (!sensor) return send(response, 404, { code: 'SENSOR_NOT_FOUND' })
        const nonce = issueNonce(store, 'sensor-reading', sensor.id, 300)
        return send(response, 201, { sensorId: sensor.id, nonce, expiresAt: Date.now() + 300_000 })
      }

      const sensorReadingMatch = url.pathname.match(/^\/api\/sensors\/([^/]+)\/readings$/)
      if (request.method === 'POST' && sensorReadingMatch) {
        const reading = await readJson(request)
        const sensor = store.prepare('SELECT id, operator_address, public_key FROM sensors WHERE id = ?').get(sensorReadingMatch[1])
        if (!sensor) return send(response, 404, { code: 'SENSOR_NOT_FOUND' })
        if (!Number.isFinite(reading.timestamp) || !Number.isFinite(reading.temperatureC) || !Number.isFinite(reading.humidityPercent) || !String(reading.nonce) || !String(reading.requestId) || !/^[0-9a-f]{128}$/i.test(String(reading.signature))) return send(response, 400, { code: 'INVALID_SENSOR_READING' })
        if (!verifyEd25519Signature(Buffer.from(sensorMessage(reading)), Buffer.from(reading.signature, 'hex'), Buffer.from(sensor.public_key))) return send(response, 422, { code: 'SENSOR_SIGNATURE_INVALID' })
        if (!consumeNonce(store, reading.nonce, 'sensor-reading', sensor.id)) return send(response, 409, { code: 'SENSOR_NONCE_USED_OR_EXPIRED' })
        const mission = store.prepare("SELECT id FROM requests WHERE id = ? AND accepted_by = ? AND status = 'accepted' AND window_starts_at <= ? AND window_ends_at >= ?").get(reading.requestId, sensor.operator_address, reading.timestamp, reading.timestamp)
        if (!mission) return send(response, 422, { code: 'READING_NOT_BOUND_TO_ACTIVE_REQUEST' })
        if (reading.temperatureC < -20 || reading.temperatureC > 60 || reading.humidityPercent < 0 || reading.humidityPercent > 100 || Math.abs(Date.now() - reading.timestamp) > 10 * 60_000) return send(response, 422, { code: 'READING_IMPLAUSIBLE_OR_STALE' })
        const payloadDigest = sha256(sensorMessage(reading))
        try {
          store.prepare('INSERT INTO sensor_readings (id, sensor_id, request_id, payload_digest, temperature_c, humidity_percent, observed_at, accepted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(opaqueId('env'), sensor.id, reading.requestId, payloadDigest, reading.temperatureC, reading.humidityPercent, reading.timestamp, Date.now())
          store.prepare("UPDATE sensors SET state = 'active', last_seen_at = ? WHERE id = ?").run(Date.now(), sensor.id)
        } catch {
          return send(response, 409, { code: 'SENSOR_PAYLOAD_REPLAYED' })
        }
        return send(response, 201, { status: 'accepted', integrity: 'valid_signature_and_nonce' })
      }

      const measurementChallengeMatch = url.pathname.match(/^\/api\/requests\/([^/]+)\/measurement-challenge$/)
      if (request.method === 'POST' && measurementChallengeMatch) {
        const session = requireSession(request, store, 'contributor')
        if (!session) return send(response, 401, { code: 'CONTRIBUTOR_AUTH_REQUIRED' })
        const mission = store.prepare("SELECT id FROM requests WHERE id = ? AND accepted_by = ? AND status = 'accepted'").get(measurementChallengeMatch[1], session.wallet_address)
        if (!mission) return send(response, 404, { code: 'ACTIVE_ASSIGNMENT_NOT_FOUND' })
        const nonce = issueNonce(store, 'connectivity-reading', `${mission.id}:${session.wallet_address}`, 300)
        return send(response, 201, { requestId: mission.id, nonce, expiresAt: Date.now() + 300_000 })
      }

      const connectivityMatch = url.pathname.match(/^\/api\/requests\/([^/]+)\/connectivity$/)
      if (request.method === 'POST' && connectivityMatch) {
        ...
        const requestedLocation = decryptLocation(mission.location_ciphertext, locationEncryptionKey)
        if (distanceMeters(requestedLocation, reading.location) > Math.max(250, reading.location.accuracyMeters)) return send(response, 422, { code: 'LOCATION_OUTSIDE_REQUEST_TOLERANCE' })
        if (reading.downloadMbps < 0 || reading.downloadMbps > 10_000 || reading.uploadMbps < 0 || reading.uploadMbps > 10_000 || reading.latencyMs < 0 || reading.latencyMs > 10_000 || reading.jitterMs < 0 || reading.jitterMs > 10_000 || Math.abs(Date.now() - reading.measuredAt) > 10 * 60_000) return send(response, 422, { code: 'CONNECTIVITY_READING_IMPLAUSIBLE_OR_STALE' })
        if (!consumeNonce(store, reading.nonce, 'connectivity-reading', `${mission.id}:${session.wallet_address}`)) return send(response, 409, { code: 'CONNECTIVITY_NONCE_USED_OR_EXPIRED' })
        const digest = sha256(connectivityMessage({ requestId: mission.id, ...reading }))
        try { store.prepare('INSERT INTO connectivity_measurements (id, request_id, payload_digest, endpoint, download_mbps, upload_mbps, latency_ms, jitter_ms, location_ciphertext, location_accuracy_m, network_type, client_context, measured_at, accepted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(opaqueId('net'), mission.id, digest, reading.endpoint, reading.downloadMbps, reading.uploadMbps, reading.latencyMs, reading.jitterMs, encryptLocation({ latitude: reading.location.latitude, longitude: reading.location.longitude }, locationEncryptionKey), reading.location.accuracyMeters, String(reading.context.networkType).slice(0, 32), String(reading.context.userAgentClass).slice(0, 100), reading.measuredAt, Date.now()) } catch { return send(response, 409, { code: 'CONNECTIVITY_PAYLOAD_REPLAYED' }) }
        return send(response, 201, { status: 'accepted', integrity: 'session_nonce_and_context_valid' })
      }
      ...
      if (request.method === 'POST' && submitMatch) {
        ...
        } catch { return send(response, 409, { code: 'REPORT_ALREADY_GENERATED' }) }
        return send(response, 201, { reportId, preview })
      }

      const previewMatch = url.pathname.match(/^\/api\/reports\/([^/]+)\/preview$/)
      if (request.method === 'GET' && previewMatch) {
        const session = requireSession(request, store, 'seeker')
        const report = session && store.prepare('SELECT preview_json, price_luna, contributor_address FROM reports WHERE id = ? AND seeker_address = ?').get(previewMatch[1], session.wallet_address)
        if (!report) return send(response, 404, { code: 'PRIVATE_REPORT_NOT_FOUND' })
        return send(response, 200, { preview: JSON.parse(report.preview_json), priceLuna: report.price_luna, contributorAddress: report.contributor_address })
      }

      const purchaseIntentMatch = url.pathname.match(/^\/api\/reports\/([^/]+)\/purchase-intent$/)
      ...
      const purchaseVerifyMatch = url.pathname.match(/^\/api\/purchases\/([^/]+)\/verify$/)
      if (request.method === 'POST' && purchaseVerifyMatch) {
        ...
        const verified = verifyPurchaseTransaction({ recipient: purchase.contributor_address, valueLuna: purchase.expected_luna, reference: purchase.reference }, transaction)
        if (!verified.included) {
          store.prepare("UPDATE purchases SET transaction_hash = ?, state = ? WHERE id = ?").run(transactionHash, verified.reason === 'NOT_INCLUDED' ? 'pending' : 'mismatched', purchase.id)
          return send(response, verified.reason === 'NOT_INCLUDED' ? 202 : 422, { state: verified.reason === 'NOT_INCLUDED' ? 'pending' : 'mismatched', reason: verified.reason })
        }
        store.prepare("UPDATE purchases SET transaction_hash = ?, state = 'included', unlocked_at = COALESCE(unlocked_at, ?) WHERE id = ?").run(transactionHash, Date.now(), purchase.id)
        store.prepare("UPDATE reports SET status = 'unlocked' WHERE id = ?").run(purchase.report_id)
        return send(response, 200, { state: 'included', reportId: purchase.report_id })
      }

      const reportMatch = url.pathname.match(/^\/api\/reports\/([^/]+)$/)
      if (request.method === 'GET' && reportMatch) {
        const session = requireSession(request, store, 'seeker')
        const report = session && store.prepare("SELECT report_json FROM reports WHERE id = ? AND seeker_address = ? AND status = 'unlocked'").get(reportMatch[1], session.wallet_address)
        if (!report) return send(response, 404, { code: 'PRIVATE_REPORT_NOT_UNLOCKED' })
      ...
      if (request.method === 'GET' && areaMatch) {
        ...
        if (!/^-?\d{1,3}\.\d{2},-?\d{1,3}\.\d{2}$/.test(cell)) return send(response, 400, { code: 'INVALID_AREA_CELL' })
      ...
      if (request.method === 'GET' && staticRoot && !url.pathname.startsWith('/api/') && !url.pathname.startsWith('/probe/')) {
        ...
        if (filePath !== staticRoot && !filePath.startsWith(`${staticRoot}${sep}`)) return send(response, 400, { code: 'INVALID_STATIC_PATH' })
        ...
        response.writeHead(200, { 'content-type': contentTypes[extname(filePath)] ?? 'application/octet-stream', 'cache-control': extname(filePath) === '.html' ? 'no-cache' : 'public, max-age=31536000, immutable' })
        return response.end(readFileSync(filePath))
      }

      return send(response, 404, { code: 'NOT_FOUND' })
    } catch (error) {
      const code = error instanceof Error && ['PAYLOAD_TOO_LARGE', 'INVALID_JSON'].includes(error.message) ? error.message : 'INTERNAL_ERROR'
